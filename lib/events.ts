

export const events = [

        {
    slug: "event-1",
    title: "Ich möchte dass meine Kinder in einer Welt ohne SPEAKEASY aufwachsen können.",
    date: "January 4, 2024",
    list: [`We kindly request your presence at the official burial ceremony of Speakeasy. Friday, January 5th 2024, from 6 to 8pm, you will have the chance to say goodbye, enjoy a final drink at the bar, and share both the good and bad memories.

We want our kids to grow up in a world without Speakeasy, and we swear that we will never do it again.

See you soon!`],
    poster: "rip.png",
        images: ["/14_rip/IMG_5322.jpg","/14_rip/IMG_5334.jpg","/14_rip/IMG_5336.jpg","/14_rip/IMG_5338.jpg","/14_rip/IMG_5339.jpg","/14_rip/IMG_5340.jpg","/14_rip/IMG_5341.jpg","/14_rip/IMG_5342.jpg","/14_rip/IMG_5343.jpg","/14_rip/IMG_5344.jpg","/14_rip/IMG_5346.jpg","/14_rip/IMG_5348.jpg"
    ]
  },

        {
    slug: "event-2",
    title: `Speakeasy and Film Länder International Preis present the premiere screening of "KLÄRWERKSWERKSFILM" (Zum ersten Mal)`,
    date: "December 16, 2023",
    list:
      ["Dem jungen Gelehrten Philip Emmanuel Schlingler wird vom Arbeitsamt als Demütigung eine Stelle im Klärwerk zugewiesen. Dort wird er vom charmanten Klärwerksbesitzer Erbrecht von Bräunling mit offenen Armen willkommen geheißen. Obwohl ihm seine neue Stelle ein beinahe luxuriöses Dasein ermöglicht entwickelt er einen Verdacht gegen den Klärwerksleiter. Wird in der Anlage wirklich nur das Abwasser gereinigt? Was ist Bräunlings großer Plan? Hä… ist er doch nicht der freundliche Großerbe, den er quasi vorgegeben hat der gewesen zu sein!?"],
    poster: "klaerwerk.png",
    images: [
      "/13_klaerwerk/photo_2024-01-04_13-54-10.jpg",
      "/13_klaerwerk/photo_2024-01-04_13-54-20.jpg",
      "/13_klaerwerk/photo_2024-01-04_13-54-25.jpg", 
      "/13_klaerwerk/photo_2024-01-04_13-54-28.jpg", 
      "/13_klaerwerk/photo_2024-01-04_13-54-32.jpg",
      "/13_klaerwerk/photo_2024-01-04_13-54-37.jpg"
    ]
  },

      {
    slug: "event-3",
    title: "Neue Musik & Altes Geld",
    date: "November 15, 2023",
    list:
      [`The Ambient Big-Band aka the Speakeasy Freibier Philharmoniker
        
        directed by:

Helene Kuschnarew
Anabelle Agbo Godeau
Kaya Pfaff
Philip Markert

`, `Night Manager:
Christoph Wiedemann`],
    poster: "neuemusik.png",
        images: [
         "/12_musik/000001.jpg",
    "/12_musik/000003.jpg",
    "/12_musik/000010.jpg",
    "/12_musik/000037.jpg",
    "/12_musik/000067.jpg",
    "/12_musik/000088.jpg",
    ]
  },
        {
    slug: "event-4",
    title: "Der Kritische Hobbykeller",
    date: "September 30 - October 1, 2023",
    list:
      [
        `29.09.`,
        `20:00 – 22:00
        Skulptur und Raum im Zeitalter der Alltagsinstallation (ohne Paulus Goerden)
Mit: Markus Ambach, Manuel Graf, Sabrina Podemski, Mara Sporn`,
        `22:00 – 00:00
Die Zukunft der Linken
Mit: H.C. Dany, M.C. Driesch (Die Linke), Alfred Hölz (MLPD), Jody Korbach

`,
        `30.09.`,
        `01:00 – 03:00
No Hard Feelings – Das Mental Health Panel
Mit: Günther Möbius, Nina Wiedemann, Alex Wissel`,
        `03:00 – 05:00
oh…oooh..ooooohkward
Tief- und Höhepunkte meines Sexlebens
Mit: Emma Rüther und Christoph Westermeier`,
        `11:00 – 13:00
Mit ihr wolln’ wir nicht, ohne sie könn’ wir nicht – Die Institution
Mit: Philip Markert, Max Mayer, Kaya Pfaff, Dr. Vivien Trommer`,
        `14:00 – 16:00
Das Gesundheitsgespräch
Mit: Sarah Lange und Arjan Stockhausen`,
        `17:00 – 19:00
Kunst und Ökonomie
Mit: Nicholas Grafia, Linn Lühn, Giulietta Ockenfuß, Jonas Roßmeißl, Murat Önen`,
        `19:00 – 20:00
Kaffeeschulung von Matthias Klein`,
        `20:00 – 22:00
Die Große Offspacekritik
Mit: Jan Wagner und Christoph Wiedemann`,
`23:00 – 01:00
Politics and Sex
Mit: Enya Burger und Ja Jess

`,
        `01.10.`,
        `03:00 – 05:00
Nie mehr "Sorry heute nicht" (über Mode)
Mit: Marlene Kollender, Philip Markert, Lukas Mogwitz, Rawaz Rasul aka Chidiki`,
        `06:00 – 08:00
You can sit with us – Die Morning Show
Mit: Annabelle Agbo Godeau, Tobias Hohn, Kaya Pfaff
`
],
    poster: "hobbykeller.png",
        images: [
          "/11_keller/000007.jpg",
    "/11_keller/000009.jpg",
    "/11_keller/000014.jpg",
    "/11_keller/000031.jpg",
    "/11_keller/000078.jpg",
    "/11_keller/000082.jpg",
    "/11_keller/000118.jpg",
    "/11_keller/000120.jpg",
    "/11_keller/IMG_4601.JPG",
    ]
  },

    {
    slug: "event-5",
    title: "Everything we know",
    date: "July 14, 2023",
    list:
      [
        `introduction by the caricature of Philip Markert`,
        `the caricature of Tobias Hohn on amourous relationships in the room`,
        `a conversation that resembles some dudes making a podcast (quote: Harkeerat Mangat)`,
        `break`,
        `“The Engagement of Alyce Hubbard-Ford and Lukas Langguth”
a sketch starring:
Christop Wiedemann as Alyce Hubbard-Ford
Philip Markert in the role of Lukas Langguth
and Tobias Hohn as Freddy the dog.`,
`followed by:`,
`The actual, real engagement of Alyce Hubbard-Ford and Lukas Langguth. Followed by standing ovations and various outrageous reactions.`,
`Night Manager:
the caricature of Christoph Wiedemann`
      ],
    poster: "everythingweknow.png",
        images: [
          "/10_everything/000047.jpg",
    "/10_everything/000053.jpg",
    "/10_everything/000130.jpg",
    ]
  },

  {
    slug: "event-6",
    title: "Desperate Living",
    date: "June 17, 2023",
    list:
      [`performances by:
Jean-Christian Pullin Aquin,
Lukas Mogwitz,
Judith Heinemann,
Maya Luna Lorieux,
Nisha Hadodo,
Nil Zengin`, `works by:
Johann Bech Jespersen,
Aske Olsen`, `night manager: Christoph Wiedemann`],
    poster: "desperateliving.png",
        images: [
          "/09_desperate/000072.jpg",
    "/09_desperate/000094.jpg",
    ]
  },
  
  {
    slug: "event-7",
    title: "Der Kalte Krieg – ein bisschen aufgewärmt",
    date: "May 24, 2023",
    list:
    [`Jonas Roßmeißl – Die Atombombe, Funktionsweise, Wartung und mehr`,
    `Karl Schumacher – Meine Geschichte mit Stanislaw Petrow und warum es in einem Krieg gegen die Ukraine nicht zu einem Einsatz von Nuklearwaffen kommen wird`,
    `Olga Hohmann – My Nuclear Family`,
    `Listening Session:
Ode to Stanislaw Petrow`
  ],
    poster: "coldwar.png",
        images: [
          "/08_kalt/000005.jpg",
    "/08_kalt/000006.jpg",
    "/08_kalt/000060.jpg",
    "/08_kalt/000097.jpg",
    "/08_kalt/000098.jpg",
    ]
  },

  {
    slug: "event-8",
    title: "Σρεακεαζγ - Club Med",
    date: "May 2, 2023",
    list:
      [`οχι μάλακα
αυτό το βράδυ ήταν εξαιρετικά άθεο και καλύτερα να μην το συζητάμε άλλο

Honestly not so much to say here. Everyone was really drunk. 
Shoutout to Philipp for the excellent bruschetti!`],
    poster: "clubmed.jpg",
        images: [
          "/07_med/000034.jpg",
    "/07_med/000046.jpg",
    "/07_med/000074.jpg",
    "/07_med/000101.jpg",
    "/07_med/000124.jpg",
    "/07_med/IMG_6040.JPG",
    ]
  },

  {
    slug: "event-9",
    title: "Speakeasy — The Beauty Within",
    date: "March 30, 2023",
    list:
    [`Sophie Urban
Worn by Johanna Terhechte`,
    `Yan Knobloch ⁃ The Slutty Secretary`,
    `Jan Wagner & Viktoria Tarak ⁃ Ghibli
Worn by Jan Wagner`,
    `Jan Wagner & Viktoria Tarak – A night at Eager Beaver
Worn by Viktoria Tarak`,
    `Judith Heinemann ⁃ On My Way`,
    `Sabrina Podemski ⁃ Matadora`,
    `Tobias Hohn und Kaspar Stöbe ⁃ Eternal Love`,
    `Jan Wagner & Viktoria Tarak – Funny Bones
Worn By Maxi Hoffmann`,
    `Julia Wilczewski`,
    `Philipp Naujoks in Coperni SS23`,
    `Helene Kuschnarew ⁃ Glücksklee`,
    `Mona Schulzek ⁃ Gebüsch
worn by Moritz Riesenbeck`,
    `Lukas Stoever`,
    `Haiqing Wang ⁃ I Love Düsseldorf x Boba`,
    `Franzi Frings ⁃ Uomo Uovo
Performed by Ulfur Loga`,
    `Sophie Urban
Worn by Johanna Terhechte`,
    `Liebesbriefe by Steffen Jopp`,
    `Yan Knobloch ⁃ The Slutty Secretary 2`,
    `Tobias Hohn ⁃ Die Kalerlake`,
    `Marlene Kollender ⁃ I am Rite`,
    `Philip Markert ⁃ Der Letzte Strohhalm`,
    `Presentation of the Rinde am Rhein X Speakeasy Collabo by Tobias Hohn, Philip Markert, Philipp Naujoks & Christoph Wiedemann`,
    `Night Manager:
Moritz Riesenbeck & Mona Schulzek`,
    `SFX:
Max Mayer`,
  ],
    poster: "the_beatuy_within 2.jpg",
        images: [
          "/06_beauty/000036.jpg",
    "/06_beauty/000109.jpg",
    "/06_beauty/000116.jpg",
    "/06_beauty/000121.jpg",
    "/06_beauty/572C12C1-4B83-4563-B6AB-C118D4EEAF2D.jpg",
    "/06_beauty/IMG_5669.JPG",
    "/06_beauty/IMG_3558.JPG",
    "/06_beauty/IMG_3560.JPG",
    "/06_beauty/IMG_3561.JPG",
    "/06_beauty/IMG_3562.JPG",
    "/06_beauty/IMG_3563.JPG",
    "/06_beauty/IMG_3564.JPG",
    "/06_beauty/IMG_3565.JPG",
    "/06_beauty/IMG_3566.JPG",
    "/06_beauty/IMG_3567.JPG",
    "/06_beauty/IMG_3568.JPG",
    "/06_beauty/IMG_3569.JPG",
    "/06_beauty/IMG_3570.JPG",
    "/06_beauty/IMG_3571.JPG",
    "/06_beauty/IMG_3572.JPG",
    "/06_beauty/IMG_3573.JPG",
    "/06_beauty/IMG_3574.JPG",
    "/06_beauty/IMG_3575.JPG",
    "/06_beauty/IMG_3576.JPG"
    ]
  },

  {
    slug: "event-10",
    title: "Die Wiedereinführung der Vapepflicht im Speakeasy",
    date: "March 11, 2023",
    list:
    [`Vape-Artist Felix von Strictly Vapez`,
    `Begrüßung durch Prof. Dr. Karl Lauterbach`,
    `Meine erste Zigarette – Christoph Wiedemann`,
    `Zaubershow von Magier Basti`,
    `a giant love – Helene Kuschnarew`,
    `Auslandskorrespondent Philip Markert trifft Keanu Reeves (gespielt von Michael Dikta) zufällig auf dem Times Square`,
    `„spelling the word intrigued“ by Mari van Stokkum`,
    `Night Manager: Annabelle Agbo-Godeau`,
    `SFX: Philipp Naujoks`,
  ],
    poster: "vapepflich.jpg",
        images: [
          "/05_vape/000015.jpg",
    "/05_vape/000019.jpg",
    "/05_vape/000043.jpg",
    "/05_vape/000048.jpg",
    "/05_vape/000063.jpg",
    "/05_vape/000071.jpg",
    "/05_vape/000073.jpg",
    "/05_vape/000079.jpg",
    "/05_vape/000083.jpg",
    "/05_vape/000085.jpg",
    "/05_vape/IMG_9672.JPG",
    ]
  },

    {
    slug: "event-11",
    title: "Gänsehaut Feeling Poor im Speakeasy",
    date: "February 11, 2023",
    list:
    [`Einführung durch den Feierschutzbeauftragten Lukas Brüggebusch`,
    `Waiting for Godeau – Rhonda Hoover, Dan Vogt & Christoph Wiedemann`,
    `A warm welcoming by Annabelle Agbo-Godeau in the role of Tobias Hohn`,
    `Konzert von Lethalgoth777`,
    `Was mein Leben reicher macht – Christoph Wiedemann`,
    `대성당들의시대 – 박은태 performed by Junho`,
    `Lea Babel & Tamas Tschaidze`,
    `Über Fotografie – Emma Rüther`,
    `Konzert von Laza911 Rosenkrieger`,
    `Eulogy to Philip Markert by Sarah Doolan`,
    `Appearance of Philip Markert’s Ghost`,
    `Preisverleihung des Speakeasy Rundgangspreises durch Philip Markert und Philipp Naujoks

-Gewinner*innen: Lukas Mogwitz, Nan 1001 Shen, Inyoung Jung, Benjamin Jakob Enders
-Preis entgegengenommen durch Prof. Francesca Raimondi`,
    `Valentinsdate-Auktion von und mit Tobias Hohn, gewinner: Kaspar Stöbe`,
    `“Unfaithful” by Mira Mann and Murat Önen`,
    `Night Manager: Moritz Riesenbeck`,
    `Artworks:
Annabelle Agbo-Godeau
Tom König & Ramon Quenders`,
    `Pausenglocke: Hugo Bell`
  ],
    poster: "gaensehaur.jpg",
        images: [
          "/04_gaense/000011.jpg",
    "/04_gaense/000021.jpg",
    "/04_gaense/000022.jpg",
    "/04_gaense/000033.jpg",
    ]
  },

    {
    slug: "event-12",
    title: `Franz von Assisi: "Speakeasy"`,
    date: "January 28, 2023",
    list:
    [`“Ford Fiesta Diesel Werbespot” by Lukas Brüggebusch & Philip Markert`,
    `“Well done” by Yoora Park`,
    `“Claps and Jumps” by Timpika W.`,
    `a video about motorcycles by Mara Wohnhaas`,
    `“Nachtfahrt Wien” von Christoph Wiedemann
    
    `,
    `Night Manager: Christoph Wiedemann`,
    `Moderation: Achim & Faina`,
    `SFX: Lukas Brüggebusch & Jorge Loureiro`],
    poster: "franzvonassisi.png",
        images: [
    "/03_franz/000134.jpg",
    "/03_franz/c5d24c61-9743-468a-824b-9f135cf20120.JPG",
    "/03_franz/IMG_2265.jpg",
    "/03_franz/IMG_2286.jpg",
    "/03_franz/IMG_2288.jpg",
                  "/03_franz/000099.jpg",
    "/03_franz/000133.jpg",
    "/03_franz/IMG_2292.jpg",
    "/03_franz/IMG_2293.jpg",
    "/03_franz/IMG_2296.jpg",
    "/03_franz/IMG_2297.jpg",
    "/03_franz/IMG_2299.JPG",
        ]
  },

    {
    slug: "event-13",
    title: "Life's hard, Speakeasy",
    date: "January 13, 2023",
    list:
    [`a film about the cruel reality of the ballet by Jackson Carroll`,
    `“The Old Days” by Jihye Lee & Areumbit Park`,
    `a film about love by Lukas Langguth`,
    `Noah Friebel – A very long kiss`,
    `a Grand Finale by Mara Jenny
    
    `,
    `Moderation: Adonis Ademes`,
    `Artworks: Krebber/Städelschule Style paintings found on the street`,
    `Fart Sounds: Philip Markert & Jorge Loureiro`],
    poster: "/lifeshard.png",
        images: [
    "/02_life/000056.jpg",
    "/02_life/000070.jpg",
    "/02_life/IMG_1906.jpg",
    "/02_life/IMG_2087.jpg",
    "/02_life/IMG_2088.jpg",
    "/02_life/IMG_2089.jpg",
    "/02_life/IMG_2090.jpg",
    "/02_life/IMG_2091.jpg",
    "/02_life/IMG_2093.jpg",
    "/02_life/IMG_2094.jpg",
    "/02_life/IMG_2100.jpg",
    ]
  },

    {
    slug: "event-14",
    title: "If words dont't come easy, Speakeasy",
    date: "December 18, 2022",
    list:
    [`“Die Geburt der Tragödie im Geiste der Musik” von Friedrich Nietzsche, gelesen von Matthias Klein`,
    `“I Don’t Give A Shit” by Nara Bak`,
    `“Harvest” by Michael Dikta & Philip Markert`,
    `Alexander Pannier – OperHH_Schoenberg_Motorrad_31Jan21`,
    `an early work of filmmaker Jürgen Lorbeer
    
    `,
    `Night Manager: Christoph Wiedemann`,
    `Moderation: Adonis Ademes`],
    poster: "/wordsdontcomeeasy.png",
  images: [
    "/01_words/000069.jpg",
    "/01_words/IMG_1918.jpg",
    "/01_words/IMG_2075 2.jpg",
    "/01_words/IMG_2076.jpg",
    "/01_words/IMG_2077.jpg",
    "/01_words/IMG_2078.jpg",
    "/01_words/IMG_2079.jpg",
    "/01_words/IMG_2080.jpg",
    "/01_words/IMG_2083.jpg",
  ]
  },
];
